...
export default function QuotationsScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (custData?.length > 0) setSelectedCustomerId(custData[0]._id || custData[0].id);
        ...
        console.error('Failed to fetch data for Quotations:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCheckout = async () => {
    if (cart.length === 0) {
      Alert.alert('Empty Cart', 'Please add items to cart before creating quotation');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={{ flex: 1, padding: designTokens.spacing.marginMobile }}>
        <Text style={styles.header}>New Quotation</Text>
      ...
      <View style={styles.checkoutSummary}>
        ...
        <TouchableOpacity style={styles.checkoutButton} onPress={handleCheckout}>
          <Text style={styles.checkoutButtonText}>Create Quotation</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
