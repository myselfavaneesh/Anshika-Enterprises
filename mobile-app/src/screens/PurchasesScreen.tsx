...
export default function PurchasesScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (suppRes.data?.length > 0) setSelectedSupplierId(suppRes.data[0]._id || suppRes.data[0].id);
        ...
        console.error('Failed to fetch data for Purchases:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      Alert.alert('Checkout Failed', error.response?.data?.error || error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={{ flex: 1, padding: designTokens.spacing.marginMobile }}>
        <Text style={styles.header}>New Purchase</Text>
