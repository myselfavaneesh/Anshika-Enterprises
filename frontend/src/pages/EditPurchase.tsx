...
export default function EditPurchase() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (id) {
          const purchaseRes = await api.get(`/purchases/${id}`);
          const purchase = purchaseRes.data;
          
          setPurchaseInvoiceNumber(purchase.purchaseInvoiceNumber);
          setSelectedSupplierId(purchase.supplierId?._id || purchase.supplierId?.id || '');
          if (purchase.supplierId) {
             setSupplierSearch(`${purchase.supplierId.name} (${purchase.supplierId.phone})`);
          }
          
          setDiscount(purchase.discount.toString());
          setTaxRate(purchase.taxRate.toString());
          setAmountPaid(purchase.amountPaid?.toString() || '0');
          setPaymentMode(purchase.paymentMode || 'CASH');

          const loadedCart = purchase.items.map((item: any) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (confirm('Purchase updated successfully. Go to Purchases list?')) {
        navigate('/purchases');
      } else {
        // Reset form
        setCart([]);
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4">
                ...
                >
                  {isSubmitting ? 'Processing...' : 'Update Purchase'}
                </Button>
              </div>
            </CardContent>
          </Card>
